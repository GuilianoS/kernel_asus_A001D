...
int tfa9xxx_haptic_dump_objs(struct seq_file *f, void *p)
{
	...
	for (i=0; i<FW_XMEM_NR_OBJECTS1; i++) {
		struct haptic_tone_object *o = (struct haptic_tone_object *)&drv->tfa->hap_data.object_table1_cache[i][0];
	...
	for (i=0; i<FW_XMEM_NR_OBJECTS2; i++) {
		struct haptic_tone_object *o = (struct haptic_tone_object *)&drv->tfa->hap_data.object_table2_cache[i][0];
...
static enum hrtimer_restart tfa_haptic_timer_func(struct hrtimer *timer)
{
	struct drv_object *obj = container_of(timer, struct drv_object,
					      active_timer);
	struct tfa9xxx *drv = container_of(obj, struct tfa9xxx, object);
...
static void tfa_haptic_pll_off(struct work_struct *work)
{
	...
	/* turn off PLL */
	tfa2_dev_set_state(drv->tfa, TFA_STATE_POWERDOWN_HAPTIC | TFA_STATE_MUTE);
...
static void tfa9xxx_haptic_clock(struct tfa9xxx *drv, bool on)
{
	...
	if (on) {
		if (drv->clk_users == 0) {
			...
			ret = tfa2_dev_clock_stable_wait(drv->tfa); /* wait for PLL */
			mutex_unlock(&drv->dsp_lock);
			if ( ret < 0 )
				dev_err(&drv->i2c->dev, "%s:error return when waiting for clock, %d\n",
						        __func__, ret);
		}
		drv->clk_users++;
...
static void tf9xxx_update_led_class(struct work_struct *work)
{
	...
	if (obj->state != STATE_STOP) {
		...
		mutex_lock(&drv->dsp_lock);
		ret = tfa2_haptic_start(drv->tfa, data, obj->index);
		mutex_unlock(&drv->dsp_lock);
		if (ret) {
                                       ...
                                       struct device_attribute *attr, char *buf)
{
	struct led_classdev *cdev = dev_get_drvdata(dev);
	struct tfa9xxx *drv = container_of(cdev, struct tfa9xxx, led_dev);
	int state = (drv->object.state != STATE_STOP);
                                          ...
                                          const char *buf, size_t count)
{
	...
	struct tfa9xxx *drv = container_of(cdev, struct tfa9xxx, led_dev);
	struct tfa2_device *tfa = drv->tfa;
	struct drv_object *obj= &drv->object;
	u32 val;
	int rc;
	int state;
                                         ...
                                         const char *buf, size_t count)
{
	...
	if (val < 0) {
		...
		if (rc < 0)
			return rc;
	} else if ((val > 0) && (val <= FW_HB_SEQ_OBJ + tfa->hap_data.seq_max)) {
		/*
		...
		*/
		...
		tfa2_haptic_obj_get(&tfa->hap_data, drv->object_index);
...
int tfa9xxx_haptic_probe(struct tfa9xxx *drv)
{
	...
	INIT_WORK(&drv->object.update_work, tf9xxx_update_led_class);
	INIT_DELAYED_WORK(&drv->pll_off_work, tfa_haptic_pll_off);

	drv->object.type = object_tone;
	...
	hrtimer_init(&drv->object.active_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	drv->object.active_timer.function = tfa_haptic_timer_func;
...
int tfa9xxx_haptic_remove(struct tfa9xxx *drv)
{
	...
	hrtimer_cancel(&drv->object.active_timer);
	...
	cancel_work_sync(&drv->object.update_work);
	cancel_delayed_work_sync(&drv->pll_off_work);
