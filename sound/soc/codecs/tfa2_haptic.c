...
int tfa2_haptic_start_object(struct i2c_client *client, struct haptic_data *data, int index)
{
	...
	rc = tfa2_i2c_write_cf_mem32(client, cmdobjsel, &cmdobjsel_index, 1,
	                             TFA2_CF_MEM_XMEM);
...
int tfa2_haptic_stop(struct tfa2_device *tfa, struct haptic_data *data, int index)
{
	...
	return tfa2_i2c_write_cf_mem32(tfa->i2c, cmdobjsel, &stop_obj,
			1, TFA2_CF_MEM_XMEM);
}
...
int tfa2_haptic_update_duration(struct haptic_data *data, int duration)
{
	...
	if (boost_length >= duration) {
		pr_err("%s: boost break length (%d) is longer then duration (%d)",
		       __func__, boost_length, duration);
		return -EINVAL;
	}
...
int tfa2_haptic_recalculate_wait(struct tfa2_device *tfa, int object)
{
	...
	if ( object >= 0 ) {
		...
		tfa2_i2c_read_cf_mem32(tfa->i2c,
		                       FW_XMEM_GENOBJECTS1 + object * FW_XMEM_OBJECTSIZE +
...
int tfa2_hap_patch_version(struct tfa2_device *tfa, char *string)
{
	...
	if (check_haptic_version && ((fw_version & FW_VERSION_MASK) != FW_VERSION)) {
		dev_err(&tfa->i2c->dev, "%s: unsupported firmware version 0x%x, expected 0x%x\n",
		        __func__, fw_version, FW_VERSION);
		return -EINVAL;
	}
...
{
	memcpy(&seq->object_table1_cache_save, &tfa->hap_data.object_table1_cache, sizeof(tfa->hap_data.object_table1_cache));
	memcpy(&seq->object_table2_cache_save, &tfa->hap_data.object_table2_cache, sizeof(tfa->hap_data.object_table2_cache));
}

void tfa2_hap_test_restore_haptic_cache(struct tfa2_device *tfa, struct tfa2_sequence *seq)
{
	if (memcmp(&tfa->hap_data.object_table1_cache, &seq->object_table1_cache_save, sizeof(tfa->hap_data.object_table1_cache))) {
		memcpy(&tfa->hap_data.object_table1_cache, &seq->object_table1_cache_save, sizeof(tfa->hap_data.object_table1_cache));
		tfa2_i2c_write_cf_mem32(tfa->i2c,
		                        FW_XMEM_GENOBJECTS1,
		                       (int *)tfa->hap_data.object_table1_cache,
		                       FW_XMEM_NR_OBJECTS1 * FW_XMEM_OBJECTSIZE,
		                       TFA2_CF_MEM_XMEM);
	}

	if (memcmp(&tfa->hap_data.object_table2_cache, &seq->object_table2_cache_save, sizeof(tfa->hap_data.object_table2_cache))) {
		memcpy(&tfa->hap_data.object_table2_cache, &seq->object_table2_cache_save, sizeof(tfa->hap_data.object_table2_cache));
		tfa2_i2c_write_cf_mem32(tfa->i2c,
		                        FW_XMEM_GENOBJECTS2,
		                        (int *)tfa->hap_data.object_table2_cache,
		                       FW_XMEM_NR_OBJECTS2 * FW_XMEM_OBJECTSIZE,
		                       TFA2_CF_MEM_XMEM);
	}
}
...
int tfa2_hap_cnt_get_sequencer(tfaContainer_t *cnt, int devidx, int seqidx, struct tfa2_sequence *seq) {
	...
	for (i=0;i<4;i++) {
		...
		switch( type ) {
		case 0:
			seq->object = kmalloc(length32, GFP_KERNEL);
			tfa2_24_to_32(seq->object, array24+3, length24-3);
			break;
		case 1:
			seq->duration = kmalloc(length32, GFP_KERNEL);
			tfa2_24_to_32(seq->duration, array24+3, length24-3);
			break;
		case 2:
			seq->level = kmalloc(length32, GFP_KERNEL);
			tfa2_24_to_32(seq->level, array24+3, length24-3);
			break;
		case 3:
			seq->freq = kmalloc(length32, GFP_KERNEL);
