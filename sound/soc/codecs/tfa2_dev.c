...
#define MIN_BATT_LEVEL 640 /* 640 corresponds to 3.4 volt */
...
static const char* tfa2_state_enum_string[] =
{
	...
	"TFA_STATE_MTP_WRITE",  /**< optimal clock depending on platform and current state */
};
...
int tfa2_dev_set_state(struct tfa2_device *tfa, enum tfa_state state) {
	...
	case TFA_STATE_CLOCK: /**< always return with clock, use OSC if no external clock  */
		...
		if ( tfa2_i2c_read_bf(tfa->i2c, tfa->bf_noclk) == 1 ) { /* use the oscillator */
			rc = tfa2_dev_set_state(tfa, TFA_STATE_OSC);
			if (rc < 0)
				break;
	...
	case TFA_STATE_OPERATING: /* Amp and Algo running */
		...
		if (tfa2_dev_cf_enabled(tfa)) {
			...
			if (tfa2_i2c_read_bf(tfa->i2c, TFA9XXX_BF_ACS) == 0 && /* warm */
				tfa2_i2c_read_bf(tfa->i2c, TFA9XXX_BF_MTPOTC) == 1 ) { /* once */
	...
	case TFA_STATE_INIT_CF: /**< coolflux HW access possible (~initcf) */
		...
		break;
	case TFA_STATE_MTP_WRITE: /**< optimal clock depending on platform and current state */
		/*  used for MTP manual copy */
		if ( manstate == 6 || manstate == 9 ) /* done if already there */
		{
			rc = 0;
			break;
		}
		/* set next state depending on type */
		manstate = (tfa->is_probus_device == 1) ?
				TFA_STATE_OPERATING : /* no initcf for probus devices */
				TFA_STATE_INIT_CF;
		rc = tfa2_dev_set_state(tfa, manstate);
		if (rc < 0)
			break;
		break;
...
int tfa2_dev_dsp_system_stable(struct tfa2_device *tfa, int *ready)
{
	...
	rc = tfa2_i2c_read_bf(tfa->i2c, tfa->bf_clks)==1;
	if (rc < 0) {
		*ready = 0;
		return rc;
	}
...
int tfa2_dev_cf_enabled(struct tfa2_device *tfa)
{
	...
	if ( tfa->is_probus_device ) { /* probus has no CFE bit */
		value = 0;
	} else {
...
//		return rc;
	...
	rc = tfa2_process_patch_file(tfa->i2c,
	                             patchLength-FW_PATCH_HEADER_LENGTH,
	                             patchBytes+FW_PATCH_HEADER_LENGTH);
...
int tfa2_i2c_dsp_init(struct tfa2_device *tfa, const char *cmd_buf,
                      size_t cmd_len)
{
	(void)tfa;
	(void)cmd_buf;
 ...
 */
int tfa2_dsp_execute(struct tfa2_device *tfa, const char *cmd_buf, size_t cmd_len,
...
int tfa2_24_to_32(int32_t * data32, uint8_t *data24, int length_bytes24)
{
	...
	for (i = 0 ; i < length_bytes24;  i += 3)  {
		int tmp = ((uint8_t) data24[i] << 16) + ((uint8_t) data24[i + 1] << 8)
		          + (uint8_t) data24[i + 2];
...
static int write_cf_mem(struct i2c_client *client, uint16_t address, int32_t *input, int size, enum tfa2_cf_mem type, enum tfa2_cf_flags flags)
{
	int idx = 0;
	uint8_t *output;
	int rc;
...
{
	return write_cf_mem(client, address, data, size, type, convert | dsp_reset);
}
...
{
	return read_cf_mem(client, address, data, size, type, convert | dsp_reset);
}
...
static int set_mtp(struct tfa2_device *tfa, uint16_t bf, uint16_t value)
{
	...
	/* Go to a state good for MTP writing */
	rc = tfa2_dev_set_state(tfa, TFA_STATE_MTP_WRITE);
	if ( rc <  0 )
		return rc;
...
restore_state:
	...
	/* restore state  */
	if (save_state == 6)
		save_state = TFA_STATE_INIT_CF;
	else if (save_state > 6)
		save_state = TFA_STATE_OPERATING;
	else
		save_state = TFA_STATE_POWERDOWN;

	rc = tfa2_dev_set_state(tfa, save_state);
	if ( rc <  0 )
		return rc;
	return 0;
}
...
void tfa2_dev_adapt_noisemode(struct tfa2_device *tfa)
{
	int i, avbatt;
	long total_bats = 0;

	if ((tfa2_i2c_read_bf(tfa->i2c, TFA9XXX_BF_PWMFREQ) == 0) ||
		(tfa2_i2c_read_bf(tfa->i2c, TFA9XXX_BF_LP1) == 1)) {
		if (tfa->verbose > 1) /* very verbose */
		 	dev_dbg(&tfa->i2c->dev,
		 	"Adapting low noise mode is not needed, condition not fulfilled!\n");
		return;
	}

	if (tfa->verbose)
		dev_dbg(&tfa->i2c->dev,"Adapting low noise mode\n");

	for (i=0; i<NR_OF_BATS; i++) {
		int bat = tfa2_i2c_read_bf(tfa->i2c,TFA9XXX_BF_BATS);
		if (tfa->verbose)
			dev_dbg(&tfa->i2c->dev,"bats[%d]=%d\n", i, bat);
		total_bats += bat;
	...
	if (avbatt <= MIN_BATT_LEVEL &&
		!tfa2_i2c_read_bf(tfa->i2c, TFA9XXX_BF_LNMODE)) {
		tfa2_i2c_write_bf(tfa->i2c,TFA9XXX_BF_LNMODE,1);
		dev_dbg(&tfa->i2c->dev,
				"\navbatt= %d--Applying high noise gain\n", avbatt);
	} else if (avbatt > MAX_BATT_LEVEL &&
				tfa2_i2c_read_bf(tfa->i2c, TFA9XXX_BF_LNMODE)) {
		tfa2_i2c_write_bf(tfa->i2c,TFA9XXX_BF_LNMODE,0);
		dev_dbg(&tfa->i2c->dev,
